...
import { getDestinationRollupByLabel } from './liveCatalog';
...
async function getBridgeRoute(form, destination, asset) {
  const assetsResponse = await fetchJson(
    `${initiaConfig.skipApiBaseUrl.replace(/\/$/, '')}/v2/fungible/assets_between_chains`,
    {
      method: 'POST',
      body: JSON.stringify({
        source_chain_id: initiaConfig.chainId,
  ...
  const assetPair = (assetsResponse.data?.assets_between_chains || []).find(
    (entry) => entry.asset_on_source?.denom === asset.denom
  );

  if (!assetPair?.asset_on_dest?.denom) {
    return {
      check: createCheck(
        'route',
        'Route availability',
  ...
  if (amountIn === '0') {
    return {
      ...
      bridgeDefaults: {
        ...
        dstDenom: assetPair.asset_on_dest.denom,
  ...
  const routeResponse = await fetchJson(
    `${initiaConfig.skipApiBaseUrl.replace(/\/$/, '')}/v2/fungible/route`,
    {
      ...
      body: JSON.stringify({
        ...
        dest_asset_denom: assetPair.asset_on_dest.denom,
        amount_in: amountIn,
  ...
  return {
    check: createCheck(
      ...
      requiresMultiStep
        ? 'A real route is available, but the bridge path expands into multiple settlement operations.'
...
export async function validateBridgeIntentLive(form) {
  ...
  const destination = await getDestinationRollupByLabel(form.destinationRollup);
