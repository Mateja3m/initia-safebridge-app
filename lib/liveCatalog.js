import { getAssetConfig, initiaConfig } from './appConfig';
...
export async function getRoutableDestinationRollups(assetLabel) {
  const asset = getAssetConfig(assetLabel);

  if (!asset) {
    throw new Error(`Unknown asset: ${assetLabel}`);
  }

  const [rollups, assetsResponse] = await Promise.all([
    getLiveDestinationRollups(),
    fetchJson(`${initiaConfig.skipApiBaseUrl.replace(/\/$/, '')}/v2/fungible/assets_from_source`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  if (!assetsResponse.ok) {
    throw new Error(
      assetsResponse.data?.message ||
        assetsResponse.data?.error ||
        `Live route catalog failed with HTTP ${assetsResponse.status}.`
    );
  }

  const reachableChains = new Set(Object.keys(assetsResponse.data?.dest_assets || {}));

  return rollups.filter((rollup) => reachableChains.has(rollup.chainId));
}
