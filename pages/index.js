...
import BridgeFormCard from '../components/BridgeFormCard';
...
function createValidationFailureResult(message) {
  return {
    ...
    checks: [
      {
        key: 'rpc',
        label: 'Source RPC responsiveness',
        status: 'fail',
        message,
      },
    ],
...
export default function HomePage() {
  ...
  const statusItems = useMemo(() => {
    ...
    const items = [
      {
        ...
        caption: wallet ? wallet.displayAddress : 'Connect OKX Wallet via InterwovenKit',
      },
      {
        label: 'Network',
    ...
    if (executionState.txHash) {
      items.push({
        label: 'Last tx hash',
        value: executionState.txHash.slice(0, 14),
